"""p0014_wowii_forest_number_bounds の LaTeX 本文。数値は必ず証明書 (cert.data) から生成する.

LaTeX 本文は ``@@KEY@@`` を差し込み口にした素の文字列で持ち、最後に一度だけ
置換する (f-string の波括弧エスケープで LaTeX を壊さないため)。
"""

from __future__ import annotations

from ..report.texescape import escape, tt

#: 族の種類の日本語名。
KIND_NAME = {"graphs": "連結グラフ (完全リスト)", "trees": "木 (完全リスト)"}

#: 本文の表に出す順序 (扱う 9 本)。
MAIN_ORDER = ["c40", "c58", "c59", "c61", "c63", "c64", "c65", "c66a", "c91"]

#: 読みが 2 通りある 2 本 (逐語 / 別読み)。
READING_ORDER = ["c66", "c66a", "c53", "c53a"]

#: 命題 5 の十分条件 (キー -> 数式表記)。本文と被覆表で同じ文字列を使う。
HYPOTHESIS = {
    "c40": r"$p \le 1$",
    "c58": r"$\ell_{\mathrm{avg}} \ge 2$",
    "c59": r"$\mathrm{res}\,(2f-2) \le f^2$",
    "c61": r"$\mathrm{diam} \le 3$",
    "c63": r"$\mathrm{dist_{even}min} \le f + 1$",
    "c64": r"$\Delta \le \alpha + 2$ または $\Delta \mid n$",
    "c65": r"$\mathrm{dist\,min}(M) \le 3$",
    "c66a": r"$m \ge \mathrm{em}\,(\Delta + 1)$ または $G$ が木",
    "c91": r"$f\,(4 - L) \le 6$",
}
...
def _name(key: str, info: dict) -> str:
    """``66 (別読み)`` のような予想の呼び名."""
    row = info[key]
    if row["reading"] == "原文どおり":
        return str(row["number"])
    return f"{row['number']} ({row['reading']})"


def _names(keys: list[str], info: dict) -> str:
    return "・".join(_name(k, info) for k in keys)


def _fam_label(fam: dict) -> str:
    if fam["kind"] == "regular":
        return f"${fam['param']}$-正則"
    if fam["kind"] == "bounded":
        return f"$\\Delta \\le {fam['param']}$"
    return KIND_NAME[fam["kind"]]


def _fam_rows(fams: list[dict]) -> str:
    rows = []
    for f in fams:
        pct = 100.0 * f["fast_hits"] / f["count"] if f["count"] else 0.0
        rows.append(
            f"{_fam_label(f)} & {f['n']} & {_n(f['count'])} & "
            f"{_n(f['fast_hits'])} ({pct:.0f}\\%) & {_n(f['exact_calls'])} & "
            f"{f['seconds']:.1f} \\\\")
    return "\n".join(rows)


def _main_rows(info: dict, tot: dict) -> str:
    rows = []
    for key in MAIN_ORDER:
        row = info[key]
        rows.append(
            f"{_name(key, info)} & ${row['rhs_tex']}$ & "
            f"{_n(tot.get(key + ':strict', 0))} & "
            f"{_n(tot.get(key + ':tight', 0))} & "
            f"{_n(tot.get(key + ':fail', 0))} \\\\")
    return "\n".join(rows)


def _reading_rows(info: dict, tot: dict, star_tot: dict) -> str:
    rows = []
    for key in READING_ORDER:
        row = info[key]
        rows.append(
            f"{row['number']} & {row['reading']} & ${row['rhs_tex']}$ & "
            f"{_n(tot.get(key + ':tight', 0))} & "
            f"{_n(tot.get(key + ':fail', 0))} & "
            f"{_n(star_tot.get(key + ':tight', 0))} & "
            f"{_n(star_tot.get(key + ':fail', 0))} \\\\")
    return "\n".join(rows)


def _cover_pct(cov: dict, key: str) -> float:
    """適用外を除いた母数に対する「成立」の割合."""
    yes = cov.get(key + ":yes", 0)
    den = yes + cov.get(key + ":no", 0) + cov.get(key + ":?", 0)
    return 100.0 * yes / den if den else 0.0


def _coverage_rows(info: dict, cov: dict) -> str:
    rows = []
    for key in MAIN_ORDER:
        rows.append(
            f"{_name(key, info)} & {HYPOTHESIS[key]} & "
            f"{_n(cov.get(key + ':yes', 0))} ({_cover_pct(cov, key):.1f}\\%) & "
            f"{_n(cov.get(key + ':no', 0))} & "
            f"{_n(cov.get(key + ':?', 0))} & "
            f"{_n(cov.get(key + ':skip', 0))} \\\\")
    return "\n".join(rows)


def _star_rows(info: dict, star_tot: dict) -> str:
    rows = []
    for key in MAIN_ORDER:
        cells = []
        for cls, label in (("tight", "等号"), ("strict", "狭義"),
                           ("fail", "反例")):
            cnt = star_tot.get(f"{key}:{cls}", 0)
            if cnt:
                cells.append(f"{label} {cnt}")
        rows.append(f"{_name(key, info)} & {'、'.join(cells) or '---'} \\\\")
    return "\n".join(rows)


def _smallest(fams: list[dict], key: str):
    """``key`` の反例のうち位数が最小のものを 1 件返す (無ければ ``None``)."""
    cand = [ex for fam in fams
            for ex in (fam["false_examples"].get(key) or [])]
    return min(cand, key=lambda e: (e["n"], e["g6"])) if cand else None


def _is_star(ex: dict) -> bool:
    r"""記録された不変量が星 $K_{1,n-1}$ の閉じた形とすべて一致するか.

    証明書は graph6 と不変量しか持たないので、同型判定の代わりに
    $m = n-1$, $\Delta = \alpha = \mathrm{res} = n-1$, $f = b = n$,
    $\mathrm{diam} = 2$ の一致を見る。一致しなければ本文は星だと書かない。
    """
    n = ex["n"]
    return (ex.get("m") == n - 1 and ex.get("Delta") == n - 1
            and ex.get("alpha") == n - 1 and ex.get("res") == n - 1
            and ex.get("f") == n and ex.get("b") == n
            and ex.get("diam") == 2)
...
いるか}を仮定が破れるグラフの個数として数え上げる。第三に @@NTOTAL@@ 個のグラフに対する
\textbf{証人つき網羅検証}を与える。$f$, $b$, $p$ の決定はいずれも NP 困難だが、
誘導森を張る頂点集合・二部部分グラフの $2$ 彩色・頂点素な奇閉路の族・道被覆の
頂点列を証人にすれば下界と上界が多項式時間で確認でき、@@NFAST@@ 個 (@@FASTPCT@@\%) は厳密解を計算せずに閉じられる。走査範囲
では予想 @@NOFAIL@@ のいずれにも反例が無く、本稿の定理・命題の破れも
@@NBAD@@ 件だった。
"""
...
だけは $b(G) \le$ 右辺である。「狭義」「等号」「反例」は走査した @@NTOTAL@@ 個
のグラフでの内訳 (予想 66 は偶数次数をもたない @@SKIP66@@ 個を除く)。}
\label{tab:conj}
\begin{tabular}{llrrr}
\toprule
WOWII & 右辺 & 狭義 & 等号 & 反例 \\
...
\item \textbf{証人つき網羅検証} (定理 \ref{thm:main})。@@NTOTAL@@ 個のグラフを
  走査し、各グラフに 4 種の証人を付けた @@NRECORDS@@ 件の記録を残す。
...
@@CE66@@
走査した @@NTOTAL@@ 個のグラフでは、逐語の読みの反例は @@N66@@ 個、別読みの
反例は @@N66A@@ 個である。@@TREE66@@
予想 53 についても同じ現象が起きる。逐語の読みには @@N53@@ 個の反例があり、
別読みには @@N53A@@ 個である。@@CE53@@ただしページ上の状態 \texttt{F} が
どちらの読みに対する反証なのかは原文からは決まらないので、本稿は 53 を
\textbf{対照}として両方の読みを併記するにとどめる (表 \ref{tab:reading})。

\begin{table}[htbp]
\centering
\caption{2 通りの読みの比較。「本体」は走査した @@NTOTAL@@ 個のグラフ、
「星族」は $K_{1,k}$ ($@@KMIN@@ \le k \le @@KMAX@@$) での件数。いずれも偶数
...
厳密計算に落とす}。走査全体で @@NFAST@@ 個 (@@FASTPCT@@\%) が証人だけで閉じ、
厳密計算の呼び出しは @@NEXACT@@ 回だった。木は全頂点集合が森かつ二部なので
...
@@FAMROWS@@
\midrule
合計 & & @@NTOTAL@@ & @@NFAST@@ (@@FASTPCT0@@\%) & @@NEXACT@@ & @@SECS@@ \\
\bottomrule
\end{tabular}
\end{table}

走査したのは、位数 $@@GMAX@@$ 以下の連結グラフ全体 (@@NGRAPHS@@ 個)、位数
$@@TMIN@@$ 以上 $@@TMAX@@$ 以下の木全体 (@@NTREES@@ 個)、位数 $10$ で最大次数
$4$ 以下の連結グラフ全体 (@@NBOUNDED@@ 個。位数 10 の連結グラフ @@DEG4TOTAL@@
個を走査して絞り込んだ)、$(n, r) \in \{@@REGPAIRS@@\}$ の連結 $r$-正則グラフ
全体 (@@NREG@@ 個) である。グラフのリストは @@SRCGRAPHS@@、木は @@SRCTREES@@、
正則グラフは @@SRCREG@@ から取った \cite{mckay, genreg}。

\begin{remark}[族は互いに素ではない]\label{rem:overlap}
表 \ref{tab:fams} の個数は\emph{走査した延べ数}である。位数 $@@TMAX@@$ 以下の
...
証明書は $K_{1,k}$ ($@@KMIN@@ \le k \le @@KMAX@@$、@@NSTARS@@ 個) を独立に
構成して扱う 9 本すべてを分類する (表 \ref{tab:star})。このうち位数が
$@@MAXLIST@@$ 以下の @@NDUP@@ 個は木または連結グラフの完全リストに既に含まれ
ており、残る @@NNEW@@ 個 ($k \ge @@KNEWMIN@@$) は走査範囲の外にある。等号が
$k$ によらず続くことが、これら 3 本 (と別読みの 66) が\textbf{定数項まで改善
できない}ことの根拠である。

\begin{table}[htbp]
\centering
\caption{星族 $K_{1,k}$ ($@@KMIN@@ \le k \le @@KMAX@@$) での分類。予想 66 は
$k$ が偶数の @@NSTAREVEN@@ 個だけが判定対象になる。}
\label{tab:star}
\begin{tabular}{rl}
\toprule
WOWII & 内訳 \\
...
@@FULLCOVERPARA@@@@WORSTPARA@@@@UNKNOWNPARA@@
...
\item $\mathrm{dist\,min}(S)$ を $|S| \le 1$ のとき $@@DISTMIN@@$ と読んだ。
  $\infty$ と読むと予想 65 は最大次数の頂点がただ 1 つのグラフのほとんどで
  偽になり、予想として意味をなさない。この読みの選択は原文からは決まらない。
\item 等号グラフの graph6 リストは族ごとに @@CAP@@ 件で打ち切っている。
  @@TRUNCPARA@@
\item 予想 53 はページ上で既に \texttt{F} (反証済み) であり、本稿は対照として
  両方の読みを走査したにすぎない。逐語の読みの反例 @@N53@@ 個が \texttt{F} の
  根拠と同じものかどうかは確認していない。
...
def build(cert) -> dict[str, str]:
    ...
    info = d["conjectures"]
    stars = d["stars"]
    star_tot = d["star_totals"]

    graphs = [f for f in fams if f["kind"] == "graphs"]
    trees = [f for f in fams if f["kind"] == "trees"]
    bounded = [f for f in fams if f["kind"] == "bounded"]
    regs = [f for f in fams if f["kind"] == "regular"]

    n_total = sum(f["count"] for f in fams)
    n_fast = sum(f["fast_hits"] for f in fams)
    n_exact = sum(f["exact_calls"] for f in fams)
    secs = sum(f["seconds"] for f in fams)
    fast_pct = 100.0 * n_fast / n_total if n_total else 0.0
    g_max = max(f["n"] for f in graphs)
    t_min = min(f["n"] for f in trees)
    t_max = max(f["n"] for f in trees)
    max_list = max(g_max, t_max)

    ks = sorted(row["k"] for row in stars)
    k_min, k_max = ks[0], ks[-1]
    dup_ks = [k for k in ks if k + 1 <= max_list]
    new_ks = [k for k in ks if k + 1 > max_list]

    no_fail = [k for k in MAIN_ORDER if tot.get(k + ":fail", 0) == 0]
    full_cover = [k for k in MAIN_ORDER
                  if cov.get(k + ":no", 0) == 0 and cov.get(k + ":?", 0) == 0]
    unknown_keys = [k for k in MAIN_ORDER if cov.get(k + ":?", 0)]
    rest = [k for k in MAIN_ORDER if k not in full_cover]
    worst = min(rest, key=lambda k: _cover_pct(cov, k)) if rest else None

    ce66 = _smallest(fams, "c66")
    ce53 = _smallest(fams, "c53")
    tree66 = sorted((ex for f in trees
                     for ex in (f["false_examples"].get("c66") or [])),
                    key=lambda e: e["n"])
    ...
    if ce66 is not None:
        ce66_text = (
            f"証明書が記録した反例のうち位数が最小のものは位数 ${ce66['n']}$ の "
            f"{tt(ce66['g6'])} で、$f = {ce66['f']}$、"
            f"$\\mathrm{{em}} = {ce66['even_mode_min']}$、"
            f"$\\overline{{d}} = {2 * ce66['m']}/{ce66['n']}$ に対して逐語の"
            f"右辺は ${_frac([ce66['rhs_num'], ce66['rhs_den']])}$ になる。")
        if _is_star(ce66):
            ce66_text += (
                f"位数・辺数・$\\Delta$・$\\alpha$・$\\mathrm{{res}}$・$f$・$b$・"
                f"$\\mathrm{{diam}}$ のいずれもが $K_{{1,{ce66['n'] - 1}}}$ の"
                f"値と一致する。")
    ce53_text = ""
    if ce53 is not None:
        ce53_text = (
            f"逐語の読みの反例のうち位数が最小のものは位数 ${ce53['n']}$・辺数 "
            f"${ce53['m']}$ の {tt(ce53['g6'])} で、"
            f"$\\mathrm{{mode_{{min}}}} = {ce53['mode_min']}$、"
            f"$f = {ce53['f']}$ に対し右辺は "
            f"${_frac([ce53['rhs_num'], ce53['rhs_den']])}$ である。")
    tree66_text = ""
    if tree66 and all(_is_star(ex) for ex in tree66):
        orders = "、".join(f"${ex['n']}$" for ex in tree66)
        tree66_text = (
            f"位数 ${t_min}$ 以上の木の中に現れた逐語の読みの反例は位数 "
            f"{orders} の {len(tree66)} 個で、いずれも記録された不変量が "
            f"$K_{{1,n-1}}$ の閉じた形と一致する。")

    full_para = ""
    if full_cover:
        names = _names(full_cover, info)
        hyps = "・".join(HYPOTHESIS[k] for k in full_cover)
        full_para = (
            f"注目すべきは予想 {names} で、走査した {_n(n_total)} 個のグラフが"
            "\\textbf{すべて}命題 \\ref{prop:partial} の仮定を満たす。つまり"
            "この範囲では網羅検証は命題以上の情報を与えておらず、逆にいえば"
            f"予想 {names} を一般に解くには、仮定が破れるグラフを走査範囲の外に"
            "探すか、仮定 (" + hyps + ") 自体を無条件に証明することになる。"
            f"後者ができれば予想 {names} は完全に解決する。\n\n")
    worst_para = ""
    if worst is not None:
        worst_para = (
            f"仮定が最も届かないのは予想 {_name(worst, info)} で、覆えたのは "
            f"{_n(cov.get(worst + ':yes', 0))} 個 "
            f"({_cover_pct(cov, worst):.1f}\\%) にとどまる。"
            f"命題 \\ref{{prop:partial}} の (66) は $m$ が $\\mathrm{{em}}$ に"
            "比べて十分大きいことを要求するので、疎で偶数次数の多いグラフには"
            "届かない。\n\n")
    unknown_para = ""
    if unknown_keys:
        names = _names(unknown_keys, info)
        cnt = sum(cov.get(k + ":?", 0) for k in unknown_keys)
        unknown_para = (
            f"「不明」が残るのは予想 {names} だけで、合計 {_n(cnt)} 個である。"
            "これは仮定に道被覆数 $p$ が現れ、証人が与えるのは $p$ の上界だけ"
            "だからである。$p$ の厳密計算は状態数 $2^n \\cdot n$ の部分集合 DP "
            "で行うため、予想の判定に不要なグラフでは省いた。省いたグラフでは"
            "\\textbf{仮定が成り立つかどうかが未確定なだけ}であり、予想 "
            f"{names} 自体の成否とは無関係である (表 \\ref{{tab:conj}} の"
            "とおり反例は無い)。\n\n")

    truncated = sorted({(fam["tag"], key) for fam in fams
                        for key, ok in fam["tight_complete"].items() if not ok})
    if truncated:
        tags = sorted({t for t, _ in truncated})
        keys = sorted({k for _, k in truncated})
        trunc_para = (
            "打ち切りが起きたのは " + "・".join(tt(t) for t in tags) + " の予想 "
            + _names(keys, info)
            + " で、表 \\ref{tab:conj} の件数自体は打ち切りの影響を受けない "
              "(件数は数え上げであり、リストは例示のためのものである)。")
    else:
        trunc_para = "走査範囲では打ち切りは起きなかった。"

    skip66 = cov.get("c66a:skip", 0)
    subs = {
        "@@NTOTAL@@": _n(n_total),
        "@@NFAST@@": _n(n_fast),
        "@@NEXACT@@": _n(n_exact),
        "@@FASTPCT@@": f"{fast_pct:.1f}",
        "@@FASTPCT0@@": f"{fast_pct:.0f}",
        "@@SECS@@": f"{secs:.0f}",
        "@@NRECORDS@@": _n(sum(f["witness_records"] for f in fams)),
        "@@WITFMT@@": escape(d["witness_format"]),
        "@@MAINROWS@@": _main_rows(info, tot),
        "@@FAMROWS@@": _fam_rows(fams),
        "@@READINGROWS@@": _reading_rows(info, tot, star_tot),
        "@@COVERROWS@@": _coverage_rows(info, cov),
        "@@STARROWS@@": _star_rows(info, star_tot),
        "@@GMAX@@": str(g_max),
        "@@TMIN@@": str(t_min),
        "@@TMAX@@": str(t_max),
        "@@MAXLIST@@": str(max_list),
        "@@NGRAPHS@@": _n(sum(f["count"] for f in graphs)),
        "@@NTREES@@": _n(sum(f["count"] for f in trees)),
        "@@NBOUNDED@@": _n(sum(f["count"] for f in bounded)),
        "@@NREG@@": _n(sum(f["count"] for f in regs)),
        "@@DEG4TOTAL@@": _n(sum(f["source_total"] or 0 for f in bounded)),
        "@@REGPAIRS@@": ", ".join(f"({f['n']}, {f['param']})" for f in regs),
        "@@SRCGRAPHS@@": escape(d["data_source"]["graphs"]),
        "@@SRCTREES@@": escape(d["data_source"]["trees"]),
        "@@SRCREG@@": escape(d["data_source"]["regular"]),
        "@@KMIN@@": str(k_min),
        "@@KMAX@@": str(k_max),
        "@@NSTARS@@": str(len(ks)),
        "@@NSTAREVEN@@": str(sum(1 for k in ks if k % 2 == 0)),
        "@@NDUP@@": str(len(dup_ks)),
        "@@NNEW@@": str(len(new_ks)),
        "@@KNEWMIN@@": str(min(new_ks)) if new_ks else "-",
        "@@N66@@": _n(tot.get("c66:fail", 0)),
        "@@N66A@@": _n(tot.get("c66a:fail", 0)),
        "@@N53@@": _n(tot.get("c53:fail", 0)),
        "@@N53A@@": _n(tot.get("c53a:fail", 0)),
        "@@SKIP66@@": _n(skip66),
        "@@CE66@@": ce66_text,
        "@@CE53@@": ce53_text,
        "@@TREE66@@": tree66_text,
        "@@NOFAIL@@": _names(no_fail, info),
        "@@NBAD@@": str(len(d["theorem_bad"])),
        "@@NPENDING@@": str(len(d["theorem_pending"])),
        "@@CAP@@": _n(d["tight_list_cap"]),
        "@@TRUNCPARA@@": trunc_para,
        "@@FULLCOVERPARA@@": full_para,
        "@@WORSTPARA@@": worst_para,
        "@@UNKNOWNPARA@@": unknown_para,
        "@@DISTMIN@@": str(d["convention"]["dist_min_singleton"]),
        "@@EVENUNDEF@@": escape(d["convention"]["even_mode_undefined"]),
        "@@CID@@": escape(cert.problem_id),
    }

    abstract, body = ABSTRACT, BODY
    for key, val in subs.items():
        abstract = abstract.replace(key, val)
        body = body.replace(key, val)
    for text, label in ((abstract, "要旨"), (body, "本文")):
        if "@@" in text:
            raise AssertionError(f"{label} に未置換の差し込み口が残っている")
    return {"ABSTRACT": abstract.strip(), "BODY": body}
