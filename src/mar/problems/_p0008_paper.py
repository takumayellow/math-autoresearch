"""p0008 の LaTeX 本文。数値は必ず証明書 (cert.data) から生成する."""
...
def _deep(fam: dict) -> int:
    """内周 4 以上のグラフ数 (球の補題が実際に使われる層)."""
    return sum(v for k, v in fam.get("girth_hist", {}).items() if int(k) >= 4)


def _rows(families: list[dict]) -> str:
    return "\n".join(
        f"{_kind(f)} & {f['n']} & {f['count']:,} & {_deep(f):,} & "
        f"{f['counts'].get('equal', 0):,} & {f['exact_calls']:,} \\\\"
        for f in families)


def _girth_rows(girth_totals: dict[str, int], total: int) -> str:
    rows = []
    for k in sorted(girth_totals, key=int):
        cnt = girth_totals[k]
        name = "なし (木)" if int(k) == 0 else f"${k}$"
        pct = 100.0 * cnt / total if total else 0.0
        rows.append(f"{name} & {cnt:,} & {pct:.3f} \\\\")
    return "\n".join(rows)


def _examples(families: list[dict], label: str) -> list[dict]:
...
def build(cert) -> dict[str, str]:
    ...
    girth_totals = d.get("girth_totals", {})
    n_deep = sum(v for k, v in girth_totals.items() if int(k) >= 4)
    deep_pct = 100.0 * n_deep / n_total if n_total else 0.0
    g_hi = max((int(k) for k in girth_totals), default=0)
    cid = _tex(cert.problem_id)

    # --- 証明書と第 3 節の定理の整合を確かめ、食い違えば本文を差し替える (原則 5)
    eq_data: dict[str, list[int]] = {}
    for f in fams:
        eq_data.update(f.get("equality_data", {}))
    off = [g6 for g6, v in eq_data.items() if v[0] != 4 or v[3] != 1 + v[1]]
    if not off and len(eq_data) == n_eq:
        eq_note = (
            f"定理 \\ref{{thm:eq}} が予言するとおり、等号が成立した "
            f"{n_eq:,} 個は\\textbf{{すべて内周 4 かつ }}"
            f"$\\mathrm{{tree}}(G) = 1 + \\Delta(G)$ であった。"
            "検証器はこの {n} 個について最大誘導木を厳密に解き直し、"
            "記録された $(\\mathrm{{girth}}, \\Delta, \\ell_{{\\max}}, "
            "\\mathrm{{tree}})$ の 4 つ組が再現することまで確かめている。"
        ).replace("{n}", f"{n_eq:,}")
    else:
        eq_note = ("\\textbf{警告}: 定理 \\ref{thm:eq} は等号が内周 4 に限る"
                   "ことを述べるが、証明書の等号リストがそれと合わない"
                   f"({len(off):,} 個が不一致)。実装を確認すること。")

    tree_eq = sum(f["counts"].get("equal", 0) for f in trees)
    if tree_eq == 0:
        tree_note = (
            f"木の族 (${t_min} \\le n \\le {t_max}$、{n_trees:,} 個) では"
            "等号が 1 個も現れない。これは定理 \\ref{thm:eq} の直接の帰結で"
            "ある: 木は閉路をもたないので内周が 0 であり、系 \\ref{cor:small} "
            "により差は常に 2 以上ある。")
    else:
        tree_note = ("\\textbf{警告}: 木は内周 0 なので等号は起こりえないが、"
                     f"証明書には {tree_eq:,} 個ある。実装を確認すること。")

    ex_parts = []
    ex_graphs = _examples(fams, "graphs")
    if ex_graphs:
        lo = ex_graphs[0]
        hi = ex_graphs[-1]
        ex_parts.append(
            f"等号グラフの最小の位数は ${lo['n']}$ で、"
            + "、".join(tt(x) for x in lo["equality_examples"][:3])
            + f" などがある。走査した最大の位数 ${hi['n']}$ では "
            f"{hi['counts'].get('equal', 0):,} 個ある。")
    ex_regs = _examples(fams, "regular")
    if ex_regs:
        r = ex_regs[0]
        ex_parts.append(
            f"正則グラフにも等号は現れる。最小の例は ${r['degree']}$-正則・"
            f"位数 ${r['n']}$ の " + tt(r["equality_examples"][0]) + " である。")
    elif regs:
        ex_parts.append(
            f"走査した連結正則グラフ ({reg_orders}、計 {n_reg:,} 個) には"
            "等号が 1 個も現れなかった。")
    ex_text = "".join(ex_parts)

    partial = [f for f in fams if not f.get("equality_complete")]
    if partial:
        # 検証器は等号リストが不完全な族があると分類の検査ごと落とす
        # (fail-closed)。したがってこの分岐に入った証明書からは PDF が
        # 生成されない。本文が黙って弱まるのを防ぐための警告として残す。
        list_note = ("\\textbf{警告}: " + "・".join(
            f"{_kind(f)}・位数 ${f['n']}$" for f in partial)
            + " の等号リストが上限を超えて不完全である。この状態では検証が"
            "失敗するので、本稿は生成されないはずである。実装を確認すること。")
    else:
        list_note = ("等号グラフはどの族についても graph6 の全リストが証明書に"
                     "載っており、検証器はそのリストの外側では\\textbf{狭義の}"
                     "不等号が成り立つことまで確かめている。")

    abstract = (
        "DeLaViña の Graffiti.pc が生成し、2005 年から未解決のまま残っている "
        "Written on the Wall II Conjecture 141 を\\textbf{解決する}。"
        "$\\mathrm{tree}(G)$ を最大誘導木の位数、$\\mathrm{girth}(G)$ を内周 "
        "(閉路がなければ $0$)、$\\ell_{\\max}(G) = \\max_v \\alpha(G[N(v)])$ を"
        "局所独立数の最大値とするとき、この予想は連結グラフ $G$ に対し "
        "$\\tfrac{1}{2}\\mathrm{girth}(G) - 1 + \\ell_{\\max}(G) \\le "
        "\\mathrm{tree}(G)$ が成り立つと主張する。本稿はこれを証明する。"
        "鍵は\\textbf{球の補題}である: $2r + 2 \\le \\mathrm{girth}(G)$ ならば"
        "半径 $r$ の球 $B_r(v)$ は必ず木を誘導する。"
        "$r = \\lfloor \\mathrm{girth}/2 \\rfloor - 1$ と取り、"
        "最大次数の頂点を中心にしてレベルを数えると "
        "$\\mathrm{tree}(G) \\ge |B_r(v^*)| \\ge \\Delta(G) + r$ が出る。"
        "内周 $\\ge 4$ では三角形がないので $\\ell_{\\max} = \\Delta$ となり、"
        "これがそのまま主張を与える。内周 $\\le 3$ は星の下界 "
        "$\\mathrm{tree} \\ge 1 + \\ell_{\\max}$ (WOWII 予想 140) で片づく。"
        "内周が奇数のときは球をさらに 1 頂点伸ばせるので、切り捨てを含む "
        "Lean 形式化よりも強い\\textbf{有理数のままの}主張が得られる。"
        "さらに等号を完全に特徴づける: 等号が成立するのは "
        "$\\mathrm{girth}(G) = 4$ かつ $\\mathrm{tree}(G) = 1 + \\Delta(G)$ の"
        "ときに限る。同じ議論から Moore 型の指数的な下界 "
        "$\\mathrm{tree}(G) \\ge 1 + \\Delta \\sum_{i<r} (\\delta-1)^i$ も従い、"
        "最小次数 $\\ge 3$ のグラフでは予想 141 が真の姿から遠く弱いことがわかる。"
        f"証明の各段は、連結グラフの完全リスト ($n \\le {g_max}$)、木 "
        f"($n \\le {t_max}$)、GENREG の連結正則グラフ、計 {n_total:,} 個の上で"
        "機械的に照合した。球の補題は内周 $\\ge 4$ の "
        f"{n_deep:,} 個すべてについて\\textbf{{全頂点}}を中心に検査し、"
        f"等号は {n_eq:,} 個で成立して、いずれも定理の述べるとおりであった。")
...
厳密計算を要したのは {n_exact:,} 個 ({exact_pct:.3f}\\%) だけである。ただし
...
$\\tfrac{{4}}{{2}} - 1 + \\Delta = 1 + \\Delta = \\mathrm{{tree}}(G)$。
...
連結 $r$-正則グラフ ($(n, r) \\in \\{{{reg_orders}\\}}$) の合計 {n_total:,} 個に
ついて、第 \\ref{{sec:proof}} 節の主張を検証器に再計算させた。
\\textbf{{(\\ref{{eq:int}}) の破れは 1 個もない。}}

内周が $4$ 以上で球の補題が実際に働くのは {n_deep:,} 個
({deep_pct:.2f}\\%) で、この全部について\\textbf{{全頂点}}を中心に
$B_r(v)$ を作り直し、木を誘導することを確かめた。同じグラフで
$|B_r(v^*)| \\ge \\Delta + r$ (定理 \\ref{{thm:main}}) と Moore 型の下界
(命題 \\ref{{prop:moore}}) も再計算した。走査に現れた最大の内周は ${g_hi}$ で
ある。内周の分布を表 \\ref{{tab:girth}} に示す。

等号は {n_eq:,} 個で成立し、残る {n_strict:,} 個では狭義に成り立った。
{eq_note}
...
\\begin{{table}}[htbp]
\\centering
\\caption{{内周の分布 (全 {n_total:,} 個)。内周 $0$ は閉路をもたないグラフ、
すなわち木を表す (本稿の走査対象は連結グラフに限る)。}}\\label{{tab:girth}}
\\begin{{tabular}}{{lrr}}
\\toprule
内周 & 個数 & 割合 (\\%) \\\\
\\midrule
{_girth_rows(girth_totals, n_total)}
\\midrule
合計 & {n_total:,} & 100.000 \\\\
\\bottomrule
\\end{{tabular}}
...
{_rows(fams)}
\\midrule
合計 & & {n_total:,} & {n_deep:,} & {n_eq:,} & {n_exact:,} \\\\
\\bottomrule
\\end{{tabular}}
\\end{{table}}
...
\\item 証明の各段 (球が木であること、レベル数え、奇内周での 1 頂点の伸長、
  Moore 型、等号の分類) を、{n_total:,} 個のグラフの上で、探索器と実装を
  共有しない検証器に再計算させた。
\\end{{enumerate}}
