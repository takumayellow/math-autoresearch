...
\item $G^2$ 読み: 証人が届かなかったのが 25 個、うち厳密に反例と
  確認したのが \textbf{16 個}。走査範囲での最小の例は \texttt{O??????\_E?K?K?E?@o?B\_} ($n = 16$) で、$\Delta(B) = 1$、$M_2$ の 2 点間の $G^2$ での最小距離は 5 だから右辺は $-1 + 1 + 5 = 5$ である。一方このグラフの葉数は厳密に $L_s = 4$ しかない。
\end{itemize}

\noindent
である。$G$ 読みの反例 891 個の内訳は連結グラフ 59 個・木 831 個・正則グラフ 1 個、$G^2$ 読みの 16 個の内訳は木 16 個である。$\mathrm{dist}_{G^2}(x,y) = \lceil \mathrm{dist}_G(x,y)/2 \rceil \le \mathrm{dist}_G(x,y)$ より $G^2$ 読みの右辺は $G$ 読みの右辺以下なので、後者の反例は前者の反例でもある (両者を足すと同じグラフを二重に数えることになる)。証明書には走査順の先頭 80 例だけを graph6 で記録し、
...
証明書ダイジェスト & \texttt{df886917931c3fb8} \\
生成日時 (UTC) & 2026-07-27T13:57:56+00:00 \\
Python & 3.10.11 \\
実行環境 & Windows 11 (build 26200) \\
リポジトリ版 & \texttt{66085f0} \\
乱数種 & 0 \\
探索時間 & 2913.0 秒 \\
