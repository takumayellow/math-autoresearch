...
\begin{abstract}
$G$ を位数 $n$ の連結グラフ、$L_s(G)$ を\textbf{葉数} (全域木がもつ葉の最大個数)、$\ell(v) = \alpha(G[N(v)])$ を\textbf{局所独立数}、$\overline{\ell}(G) = \frac{1}{n}\sum_v \ell(v)$ をその平均とする。Graffiti.pc の未解決予想 \emph{Written on the Wall II} 予想 2 (以下、予想 \ref{conj:wowii2}) は $2(\overline{\ell}(G) - 1) \le L_s(G)$ を主張する。出発点は\textbf{二重星定理}: 連結グラフの\textbf{任意の辺} $uv$ に対し $L_s(G) \ge |N(u) \cup N(v)| - 2$ (定理 \ref{thm:doublestar})。これは出題者自身がリスト \cite{wowii} の予想 178 の注記で「容易に導ける」と述べている\textbf{既知}の下界であり、本稿では自己完結のために「部分木を全域木へ延長しても葉は減らない」という一行の補題からの証明を与える。本稿の寄与は次の 3 つである。第 1 に、この定理が予想 \ref{conj:wowii2} を\textbf{全域木の最適化を含まない局所的な}主張へ帰着させることを指摘し、その主張を予想 B' $\max_{uv \in E} |N(u) \cup N(v)| \ge 2\overline{\ell}(G)$ として提出する (予想 \ref{conj:bprime})。B' は予想 \ref{conj:wowii2} を含意する (系 \ref{cor:bprime})。第 2 に\textbf{共分散定理} (定理 \ref{thm:cov}): 次数と局所独立数の共分散が非負、すなわち $\sum_v (\deg v - \overline{d})\ell(v) \ge 0$ ならば、B' より強い平均版 A (予想 \ref{conj:avg}) が成り立つ。核心は補題 \ref{lem:sumbound} $\sum_{uv \in E} |N(u) \cup N(v)| \ge \sum_v \deg(v)\,\ell(v)$ で、これは各頂点で「近傍の点被覆数 $\times$ 次数 $\ge$ 近傍の辺数」に分解する。三角形をもたないグラフは $\ell = \deg$ で共分散が分散になるので、この定理の系として 2 行で片づく (系 \ref{cor:trianglefree})。第 3 に、自明帯 $\overline{\ell} \le 2$ と $\Delta$ 帯 $2\overline{\ell} \le \Delta + 2$ で予想 \ref{conj:wowii2} を証明し (定理 \ref{thm:zones})、これらと共分散帯を合わせると走査範囲の 99.9999\% が定理として閉じることを示す。さらに $L_s$ の古典的な下界 $l(n,3) \ge n/4 + 2$, $l(n,4) \ge (2n+8)/5$ を借りると帯が 2 つ増えて (定理 \ref{thm:mindeg}、とくに $n \ge 8$ の連結な立方体グラフはすべてここで閉じる)、覆う割合は 99.99999\% になる。連結グラフの完全リスト ($n \le 10$)、木 ($11 \le n \le 20$)、GENREG の連結正則グラフ、計 13,402,242 個について、各グラフに\textbf{葉集合 1 つ}を証人として付け、探索器とグラフ計算を共有しない検証器に (i) 補集合が連結支配集合であること、(ii) 予想 \ref{conj:wowii2}、(iii) 二重星定理、(iv) 予想 B'、(v) 予想 A を再計算させた。走査した 13,402,242 個すべてで予想 \ref{conj:wowii2} は成立し、等号は 13 個で成立した。本稿が新たに提出する予想 \ref{conj:bprime} にも、より強い予想 \ref{conj:avg} にも反例はなく、等号はそれぞれ 8,796 個、8,796 個で成立した。予想 \ref{conj:wowii2} 自体は\textbf{一般には未解決のままである}。
\end{abstract}
...
解放される} (実際、本稿の走査でも $L_s$ を厳密に解いたのは 13 回、
全体の 0.0001\% だけである)。$\ell$ 自体は一般には NP 困難である
...
$\delta \ge 3$ & 残り & 残りの割合 (\%) \\
\midrule
連結グラフ & 2 & 1 & 1 & 0 & 0 & 0 & 0 & 0 & 0 \\
連結グラフ & 3 & 2 & 2 & 0 & 0 & 0 & 0 & 0 & 0 \\
連結グラフ & 4 & 6 & 6 & 0 & 0 & 0 & 0 & 0 & 0 \\
連結グラフ & 5 & 21 & 20 & 1 & 0 & 0 & 0 & 0 & 0 \\
連結グラフ & 6 & 112 & 96 & 14 & 2 & 0 & 0 & 0 & 0 \\
連結グラフ & 7 & 853 & 570 & 276 & 7 & 0 & 0 & 0 & 0 \\
連結グラフ & 8 & 11,117 & 4,587 & 6,507 & 23 & 0 & 0 & 0 & 0 \\
連結グラフ & 9 & 261,080 & 45,253 & 215,593 & 232 & 1 & 0 & 1 & 0.0004 \\
連結グラフ & 10 & 11,716,571 & 523,962 & 11,190,131 & 2,466 & 12 & 0 & 0 & 0 \\
3-正則 & 12 & 85 & 3 & 37 & 45 & 0 & 0 & 0 & 0 \\
3-正則 & 14 & 509 & 3 & 135 & 371 & 0 & 0 & 0 & 0 \\
3-正則 & 16 & 4,060 & 5 & 789 & 3,266 & 0 & 0 & 0 & 0 \\
3-正則 & 18 & 41,301 & 11 & 7,489 & 33,801 & 0 & 0 & 0 & 0 \\
4-正則 & 11 & 265 & 2 & 190 & 73 & 0 & 0 & 0 & 0 \\
4-正則 & 12 & 1,544 & 7 & 1,005 & 532 & 0 & 0 & 0 & 0 \\
4-正則 & 13 & 10,778 & 5 & 5,853 & 4,920 & 0 & 0 & 0 & 0 \\
5-正則 & 12 & 7,848 & 6 & 7,499 & 343 & 0 & 0 & 0 & 0 \\
6-正則 & 11 & 266 & 2 & 263 & 1 & 0 & 0 & 0 & 0 \\
木 & 11--20 & 1,345,823 & 1,345,823 & 0 & 0 & 0 & 0 & 0 & 0 \\
\bottomrule
\end{tabular}
\end{table}

走査した 13,402,242 個のうち、自明帯が 1,920,364 個、$\Delta$ 帯が
11,435,782 個、共分散帯が 46,082 個 (0.344\%) で、合わせて
13,402,228 個 (99.9999\%) が\textbf{本稿の定理だけで}閉じる。
定理 \ref{thm:mindeg} がさらに
$\delta \ge 4$ 帯で 13 個、$\delta \ge 3$ 帯で 0 個
(合わせて 0.0001\%) を閉じるので、引用した下界を認めれば
13,402,241 個 (99.99999\%) が定理として片づく。残るのは
1 個 (0.00001\%) で、そこでは証人つきの機械照合しか
持っていない。
...
走査した 13,402,242 個すべてで予想 \ref{conj:wowii2} は成立し、等号は 13 個で成立した。本稿が新たに提出する予想 \ref{conj:bprime} にも、より強い予想 \ref{conj:avg} にも反例はなく、等号はそれぞれ 8,796 個、8,796 個で成立した。
...
\midrule
連結グラフ & 2 & 1 & 1 & 0 & 0 & 0 & 1 & 1 \\
連結グラフ & 3 & 2 & 2 & 0 & 0 & 0 & 0 & 0 \\
連結グラフ & 4 & 6 & 5 & 0 & 1 & 1 & 1 & 1 \\
連結グラフ & 5 & 21 & 20 & 0 & 1 & 1 & 1 & 1 \\
連結グラフ & 6 & 112 & 110 & 0 & 2 & 2 & 2 & 2 \\
連結グラフ & 7 & 853 & 852 & 0 & 1 & 1 & 1 & 1 \\
連結グラフ & 8 & 11,117 & 11,113 & 1 & 3 & 3 & 4 & 4 \\
連結グラフ & 9 & 261,080 & 261,079 & 0 & 1 & 1 & 1 & 1 \\
連結グラフ & 10 & 11,716,571 & 11,716,567 & 1 & 3 & 3 & 10 & 10 \\
木 & 11 & 235 & 235 & 0 & 0 & 0 & 0 & 0 \\
木 & 12 & 551 & 551 & 0 & 0 & 0 & 0 & 0 \\
木 & 13 & 1,301 & 1,301 & 0 & 0 & 0 & 0 & 0 \\
木 & 14 & 3,159 & 3,159 & 0 & 0 & 0 & 0 & 0 \\
木 & 15 & 7,741 & 7,741 & 0 & 0 & 0 & 0 & 0 \\
木 & 16 & 19,320 & 19,320 & 0 & 0 & 0 & 0 & 0 \\
木 & 17 & 48,629 & 48,629 & 0 & 0 & 0 & 0 & 0 \\
木 & 18 & 123,867 & 123,867 & 0 & 0 & 0 & 0 & 0 \\
木 & 19 & 317,955 & 317,955 & 0 & 0 & 0 & 0 & 0 \\
木 & 20 & 823,065 & 823,065 & 0 & 0 & 0 & 0 & 0 \\
3-正則 & 12 & 85 & 77 & 8 & 0 & 0 & 22 & 22 \\
3-正則 & 14 & 509 & 493 & 16 & 0 & 0 & 110 & 110 \\
3-正則 & 16 & 4,060 & 4,022 & 38 & 0 & 0 & 792 & 792 \\
3-正則 & 18 & 41,301 & 41,218 & 83 & 0 & 0 & 7,805 & 7,805 \\
4-正則 & 11 & 265 & 264 & 1 & 0 & 0 & 2 & 2 \\
4-正則 & 12 & 1,544 & 1,539 & 5 & 0 & 0 & 12 & 12 \\
4-正則 & 13 & 10,778 & 10,772 & 6 & 0 & 0 & 31 & 31 \\
5-正則 & 12 & 7,848 & 7,847 & 0 & 1 & 1 & 1 & 1 \\
6-正則 & 11 & 266 & 266 & 0 & 0 & 0 & 0 & 0 \\
\midrule
...
$\max_{uv \in E}|N(u) \cup N(v)|$。}\label{tab:eq}
...
入らない層である。走査範囲ではこの層は 1 個 (0.00001\%)
...
閉じず、さらに共分散も負だったのは 14 個あるが、その中に予想
\ref{conj:avg} の反例は 0 個しかない。つまり共分散が負の
グラフでも A はたいてい成り立っており、A を閉じるには共分散を経由しない
議論が要る。この 14 個は走査順に全部書き出せる: \texttt{H?\textasciitilde{}vfbk} ($n = 9$、どの帯にも入らない)、\texttt{H?\textasciitilde{}vfrs} ($n = 9$、$\delta \ge 4$ 帯)、\texttt{I?bfbpwv?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?bfbr[\}?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?brvRwuO} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?brvRw\{O} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?brvrwz?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?bvbrwr\_} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?o|\textasciicircum{}bo\{\_} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?qvf`\{\textasciicircum{}?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?rF`zw|?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?rff\_\{n?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?rvdpx\}?} ($n = 10$、$\delta \ge 4$ 帯)、\texttt{I?zVfa\{\textasciicircum{}?} ($n = 10$、$\delta \ge 4$ 帯)。どれも $n$ が小さく次数がそろっており、共分散が負になるのはわずかな差である。検証器はこの並びを帯の判定からやり直して突き合わせている。
また補題
\ref{lem:sumbound} が押さえているのは辺上の\emph{平均}なので、$f(G)$ が
平均に近いグラフ (正則グラフなど) では B' への余裕がほとんど残らない
(実際、予想 \ref{conj:avg} の等号は 8,796 個で成立している)。

定理 \ref{thm:mindeg} で閉じた 13 個については、依拠して
...
\ref{conj:avg} には走査範囲で反例が 0 個しかない。すなわち
...
証明書ダイジェスト & \texttt{68bf8219a793ec46} \\
生成日時 (UTC) & 2026-07-27T05:48:01+00:00 \\
Python & 3.10.11 \\
実行環境 & Windows 11 (build 26200) \\
リポジトリ版 & \texttt{7e7f670} \\
乱数種 & 0 \\
探索時間 & 1265.8 秒 \\
...
\bibitem{storer} J. A. Storer, Constructing full spanning trees for cubic graphs, Inform. Process. Lett. 13 (1981) 8--11.
\bibitem{kw1991} D. J. Kleitman, D. B. West, Spanning trees with many leaves, SIAM J. Discrete Math. 4 (1991) 99--106. $l(n,3) \ge n/4 + 2$ (Storer / Linial--Sturtevant の再証明と最小次数 3 への拡張) と $l(n,4) \ge (2n+8)/5$。
