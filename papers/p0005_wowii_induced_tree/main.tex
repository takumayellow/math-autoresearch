...
\title{最大誘導木の距離下界: Written on the Wall II 予想 142・144・146 の証人付き同時網羅検証と等号グラフの完全分類}
...
\begin{abstract}
DeLaViña の Graffiti.pc が生成した予想 Written on the Wall II Conjecture 142・144・146 を扱う (142 と 144 は 2026 年 7 月 27 日時点で未解決。146 は 7 月 21 日付で解決済みであることが走査後に判明した。注意 \ref{rem:146})。いずれも最大誘導木の位数 $\mathrm{tree}(G)$ を距離量で下から抑える主張であり、それぞれ $\frac{2}{3}\mathrm{girth}(G) + \mathrm{ecc}(B)$、$\mathrm{girth}(G) - 1 + \mathrm{ecc}^{\circ}(C)$、$2\,\mathrm{ecc}(B)/\mathrm{rad}(G^2)$ を下界に置く ($B$ は境界、$C$ は中心)。連結グラフの完全リスト ($n \le 10$)、木の完全リスト ($n \le 20$)、および GENREG の連結正則グラフ、計 13,402,242 個に対して 3 予想を同時に網羅検証した。反例は存在せず、等号はそれぞれ 1,375・203・13,043 個で成立する。$\mathrm{tree}(G)$ の計算は NP 困難だが、3 予想の左辺はいずれも多項式時間で厳密に計算でき、右辺は共通なので、\textbf{木を誘導する頂点集合 1 つ}を証人として与えれば 3 本すべてが線形時間で確認できる。さらに手計算により、$\mathrm{tree}(G) \ge \mathrm{diam}(G)+1$ と $\mathrm{tree}(G) \ge \mathrm{girth}(G)-1$ から \textbf{予想 142 と 144 が内周 $3$ 以下のグラフと自己中心グラフで成り立つ}ことを示し、予想 146 の反例は $\mathrm{rad}(G) = 2$, $\mathrm{diam}(G) = 4$, $\mathrm{ecc}(B) = 3$, $\mathrm{tree}(G) = 5$ を同時に満たさねばならないことを示す。網羅検証はこの残った場合に反例が無いことを確認する。
\end{abstract}

\noindent\textbf{キーワード:} graph theory、induced tree、girth、eccentricity、graph square、Graffiti.pc、open problem、certificate
...
求める。走査した 13,402,242 個のうち厳密解に進んだのは 23,925 個
(0.179\%) にすぎない。
...
14,621 件 (グラフと予想の組) の分だけである。等号グラフを 1 個でも隠せば、
...
(66,656 個) のいずれにおいても、予想 \ref{conj:142}, \ref{conj:144},
\ref{conj:146} はすべて成立する。とくにこれら 3 予想の反例 $G$ が
存在するならば $n \ge 11$ であり、$G$ が木ならば
$n \ge 21$ である。
\end{theorem}
...
\texttt{p0005\_wowii\_induced\_tree.json} と証人ファイルに対して検証器が実行し、
...
連結グラフ & 4 & 6 & 3 & 4 & 0 & 4 \\
連結グラフ & 5 & 21 & 7 & 8 & 1 & 9 \\
連結グラフ & 6 & 112 & 14 & 14 & 6 & 20 \\
連結グラフ & 7 & 853 & 39 & 25 & 32 & 59 \\
連結グラフ & 8 & 11,117 & 101 & 31 & 178 & 234 \\
連結グラフ & 9 & 261,080 & 294 & 50 & 1,272 & 1,727 \\
連結グラフ & 10 & 11,716,571 & 916 & 70 & 11,554 & 21,871 \\
木 & 11 & 235 & 0 & 0 & 0 & 0 \\
木 & 12 & 551 & 0 & 0 & 0 & 0 \\
木 & 13 & 1,301 & 0 & 0 & 0 & 0 \\
木 & 14 & 3,159 & 0 & 0 & 0 & 0 \\
...
木 & 16 & 19,320 & 0 & 0 & 0 & 0 \\
木 & 17 & 48,629 & 0 & 0 & 0 & 0 \\
木 & 18 & 123,867 & 0 & 0 & 0 & 0 \\
木 & 19 & 317,955 & 0 & 0 & 0 & 0 \\
木 & 20 & 823,065 & 0 & 0 & 0 & 0 \\
3-正則 & 12 & 85 & 0 & 0 & 0 & 0 \\
3-正則 & 14 & 509 & 0 & 0 & 0 & 0 \\
3-正則 & 16 & 4,060 & 0 & 0 & 0 & 0 \\
3-正則 & 18 & 41,301 & 0 & 0 & 0 & 0 \\
4-正則 & 11 & 265 & 0 & 0 & 0 & 0 \\
4-正則 & 12 & 1,544 & 0 & 0 & 0 & 0 \\
4-正則 & 13 & 10,778 & 0 & 0 & 0 & 0 \\
5-正則 & 12 & 7,848 & 0 & 0 & 0 & 0 \\
6-正則 & 11 & 266 & 0 & 0 & 0 & 0 \\
\midrule
\multicolumn{2}{l}{合計} & 13,402,242 & 1,375 & 203
& 13,043 & 23,925 \\
\bottomrule
\end{tabular}
\end{table}
...
予想 \ref{conj:142} で 1,375 個、予想 \ref{conj:144} で
203 個、予想 \ref{conj:146} で 13,043 個あり、
その全リストが証明書に含まれる。走査した正則グラフの族 (いずれも $n \ge 11$) には、3 予想のどれについても等号が 1 個も現れない。
\end{corollary}
...
等号は起こらない。両者を合わせると、走査した 13,402,242 個における等号の集合が
確定する。
\end{proof}

等号グラフの例を graph6 表記で挙げる: 予想 142 では位数 $10$ の \texttt{I??CBb\_ZG}、\texttt{I??CBps\{O}、\texttt{I??E@aKgW}、\texttt{I??E@aKkG}、予想 144 では位数 $10$ の \texttt{I??CBbGH\_}、\texttt{I??E@agD\_}、\texttt{I??E@agT?}、\texttt{I?AA@agT?}、予想 146 では位数 $10$ の \texttt{I???F?[uw}、\texttt{I???FB\textbackslash{}uo}、\texttt{I???FbLyo}、\texttt{I???Fbkyw} である。

系 \ref{cor:tree} が予言するとおり、走査した木の族における 予想 142・144 の等号グラフは 0 個であった (表 \ref{tab:fams})。予想 146 についても、木の族には等号が 1 個も現れていない。ただしこちらは系 \ref{cor:tree} の射程外で、木が予想 146 の等号を達成し得ないことは本稿では証明していない。
...
McKay \cite{mckay} が $n \le 10$ まで、木は $n \le 22$ まで公開して
おり、本稿は前者の全体と後者の $n \le 20$ を走査した。正則グラフは
...
\begin{center}\texttt{python -m mar verify p0005\_wowii\_induced\_tree}\end{center}

\begin{center}\begin{tabular}{ll}\toprule
証明書ダイジェスト & \texttt{24b79714410310f6} \\
生成日時 (UTC) & 2026-07-26T15:21:57+00:00 \\
Python & 3.10.11 \\
実行環境 & Windows 11 (build 26200) \\
リポジトリ版 & \texttt{5fe10b2} \\
乱数種 & 0 \\
探索時間 & 3215.8 秒 \\
...
\bibitem{formalconj} Google DeepMind, formal-conjectures: FormalConjectures/WrittenOnTheWallII/GraphConjecture{142,144,146}.lean (2026-07-26 取得). \url{https://github.com/google-deepmind/formal-conjectures}
